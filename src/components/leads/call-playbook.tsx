...
export function CallPlaybook({ contactName, agentName }: CallPlaybookProps) {
  ...
  return (
    <section aria-label="Call playbook" className="mb-4 rounded-xl border border-primary/25 bg-card p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            <BookOpenCheck aria-hidden className="size-4 text-primary" />
            <h2 className="text-sm font-extrabold">{playbook.title}</h2>
          </div>
          <p className="mt-1 text-sm text-muted-foreground">{playbook.goal}</p>
        </div>
      ...
      <div className="mt-4 rounded-lg border border-primary/20 bg-primary/5 p-3">
        <p className="text-xs font-bold tracking-wide text-primary uppercase">Next line · Open</p>
        <p className="mt-1 text-sm font-semibold leading-6">“{playbookGreeting(contactName, agentName)}”</p>
      </div>
        ...
        {playbook.stages.map((stage) => (
          <details key={stage.id} className="rounded-lg border bg-background px-3">
            ...
            <div className="border-t pb-3 pt-2">
              <p className="text-sm leading-6">“{stage.id === "open" ? playbookGreeting(contactName, agentName) : stage.line}”</p>
      ...
      <div className="mt-3 grid gap-3 md:grid-cols-2">
        <div className="rounded-lg bg-muted p-3">
          <h3 className="text-xs font-bold tracking-wide uppercase">Booking handoff</h3>
          <p className="mt-1 text-sm leading-6 text-muted-foreground">{playbook.bookingHandoff}</p>
        </div>
        <div className="rounded-lg bg-muted p-3">
          <h3 className="text-xs font-bold tracking-wide uppercase">No-show recovery</h3>
          <p className="mt-1 text-sm leading-6 text-muted-foreground">{playbook.noShowRecovery}</p>
        </div>
      </div>
